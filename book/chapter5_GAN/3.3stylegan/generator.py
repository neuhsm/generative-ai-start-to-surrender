...
class SynthesisModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_res_log2 = int(np.log2(self.init_res)) # 512-> 9, 128 -> 7
        self.resolution = resolution
        ...
        self.pth_to_tf_var_mapping = {'lod': 'lod'} # 为了实现torch 和tensorflow 模型之间的转化

        # lod 是 "Level of Detail" (细节层级)的缩写，这是StyleGAN中用于渐进式训练(progressive training)的一个重要参数。它主要有以下几个作用：
        # 控制生成图像的分辨率：
        # lod=0 时生成最高分辨率的图像
        # lod=1 时降低一个层级的分辨率
        # lod=2 时再降低一个层级，以此类推
        # 支持平滑过渡：
        # 当 lod 是整数时，直接输出对应分辨率的图像
        # 当 lod 是小数时(比如1.5)，会在两个相邻分辨率之间进行插值混合，实现平滑过渡
        # 这个过渡是通过 alpha 混合实现的：alpha = np.ceil(lod) - lod
        # 用于渐进式训练：
        # 训练开始时使用较大的 lod 值，生成低分辨率图像
        # 随着训练进行，逐渐降低 lod 值，图像分辨率逐步提高
        # 这种渐进式训练方式可以使模型先学习图像的大体结构，再逐步学习细节，提高训练稳定性和效果
        # 这个参数可以在训练过程中动态调整，从而实现渐进式训练和不同分辨率图像的生成。
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            res = 2 ** res_log2
            block_idx = res_log2 - self.init_res_log2  # 每个block_idx 对应一个不同分辨率的处理块


            # First convolution layer for each resolution.
            layer_name = f'layer{2 * block_idx}' # layer0
    ...
    def forward(self, wp, lod=None, randomize_noise=False):
        ...
        # 训练开始时：LOD = 6（最低分辨率8x8）
        # 随着训练进行：LOD逐渐减小
        # 训练结束时：LOD = 0（最高分辨率512x512）
        # 每个分辨率级别：

        # 稳定阶段（600k图像）：LOD保持整数值
        # 过渡阶段（600k图像）：LOD线性插值到下一级
        # 例如：从LOD=6到LOD=5的过渡期间，LOD可能是5.7、5.3等
        # 这种渐进式训练策略：
...
class ConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.position == 'const_init': # leanable constant as the input resolution
    ...
    def forward(self, x, w, randomize_noise=False):
        ...
        # 先 对输入加入noise, 再进行activation, instance norm, style
        return x, style
