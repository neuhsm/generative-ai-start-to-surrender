...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, **layer_kwargs):
        ...
        ws = ws.to(torch.float32).unbind(dim=1)  # 根据dim=1进行拆分,返回list

        # Execute layers.
        x = self.input(ws[0]) # 第一层，第一层决定初始输入
        for name, w in zip(self.layer_names, ws[1:]): # 每一层的输入包含上一层的输出和当前层的隐空间向量
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        # 将输入的因空间向量映射到中间隐空间
        img = self.synthesis(ws, update_emas=update_emas, **synthesis_kwargs) # 根据隐空间生成图片
        return img
